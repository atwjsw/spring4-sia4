...
public class Neo4jTest {
    ...
    @Test
    public void testNeo4jTemplateCount() {
        assertEquals(0, neo4j.count(Order.class));
    }

    @Test
    public void testNeo4jSave() {
        Order order = createAnOrder();
        System.out.println(order);

        // Saving an order
        Order savedOrder = neo4j.save(order);
        assertEquals(1, neo4j.count(Order.class));
        System.out.println(savedOrder);
    }

    @Test
    public void testNeo4jTemplateCountAndSave() {
        assertEquals(0, neo4j.count(Order.class));
        Order order = createAnOrder();
        System.out.println(order);
        ...
        Order fetchedOrder = neo4j.findOne(savedOrder.getId(), Order.class);
